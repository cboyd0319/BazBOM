...
**Status:** ✅ AVAILABLE AS LIBRARY API
...
**Capabilities:**
- ✅ Full Java class file constant pool parsing
- ✅ Method signature extraction
- ✅ Field signature extraction
- ✅ Class name resolution
- ✅ API change detection (added/removed methods, fields, classes)
...
**Capabilities:**
- ✅ Spring Boot 2.x → 3.x migrations
- ✅ Log4j 1.x → 2.x migrations
- ✅ JSON configuration comparison
- ✅ Recursive key change detection
...
**Known migrations:**
- Spring Boot: datasource initialization-mode → sql.init.mode
- Spring Boot: JPA hibernate ID generator mapping removal
- Log4j: Complete configuration rewrite for 1.x → 2.x
...
**Integration points:**
- ✅ Instantiated in `UpgradeAnalyzer::new()` (analyzer.rs:34)
- ✅ Queried in `analyze_upgrade()` (analyzer.rs:109-111)
- ✅ Automatically populates `success_rate` field

**Data included:**
- Spring Boot 2.7→3.0: 89% success rate (1400 attempts)
- Jackson 2.13→2.14: 98% success rate (2850 attempts)
- Log4j 2.17→2.18: 99% success rate (3220 attempts)

**Database location:** `~/.cache/bazbom/community/upgrade_data.json`

**Capabilities:**
- ✅ Local database with caching
...
**Public API:**
```rust
use bazbom_reports::pdf::{generate_pdf, is_pdf_available};
use bazbom_reports::{ReportGenerator, SbomData, VulnerabilityFindings, PolicyStatus};

// Check if PDF generation is available
assert!(is_pdf_available()); // Returns true!

// Generate a PDF report
let generator = ReportGenerator::new(sbom, vulnerabilities, policy);
generate_pdf(&generator, Path::new("report.pdf"))?;
```

**Implementation details:**
- Uses `printpdf` crate for robust PDF generation
- Built-in Helvetica fonts (no external font dependencies)
- Professional A4 document layout with multiple pages
- Automatic page breaks when content exceeds page capacity

**PDF Content includes:**
- Title page with project info and security score
- Executive summary with key findings
- Risk assessment based on security score
- Detailed vulnerability sections (Critical, High, Medium, Low)
- SBOM statistics (dependency counts)
- Policy compliance status

**Dependencies:**
- `printpdf = "0.7"` - Core PDF generation library
- `lopdf = "0.32"` - Low-level PDF handling
- `headless_chrome = "1.0"` - Optional Chrome integration (feature-gated)

**Features:**
- Color-coded security scores (green/orange/red)
- Comprehensive vulnerability details with CVSS scores
- KEV and reachability flags
- Fixed version recommendations
- Multi-page support with automatic pagination

**Test coverage:** 2 tests (availability check + full generation test)

---

## 🔌 How Features Are Exposed
...
```bash
✅ bazbom-polyglot          (Yarn/pnpm parsers)
✅ bazbom                   (JAR bytecode)
✅ bazbom-upgrade-analyzer  (Config migration, Community data)
✅ bazbom-reports          (PDF infrastructure)
```
...
### Stable APIs (v6.5.0+)
- ✅ `bazbom_upgrade_analyzer::CommunityDatabase`
- ✅ `bazbom_upgrade_analyzer::UpgradeSuccessData`
- ✅ `bazbom::shading::compare_jars_for_breaking_changes`
- ✅ `bazbom::shading::ApiChange`
- ✅ `bazbom::shading::ApiChangeType`
- ✅ `bazbom_reports::pdf::generate_pdf`
- ✅ `bazbom_reports::pdf::is_pdf_available`

### Beta APIs (may change)
- ⚠️ `bazbom_upgrade_analyzer::breaking_changes::config::*`
...
1. ✅ Yarn.lock parsing - Used in production scanning
2. ✅ pnpm-lock.yaml parsing - Used in production scanning
3. ✅ JAR bytecode comparison - Available as stable library API
4. ✅ Config migration detection - Available as library API
5. ✅ Community upgrade data - Integrated into UpgradeAnalyzer
6. ✅ PDF generation - Fully implemented with printpdf
