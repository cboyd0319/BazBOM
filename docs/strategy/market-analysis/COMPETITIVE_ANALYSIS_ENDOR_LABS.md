...
flowchart TD
    Root["my-app:1.0.0
Runtime: JVM 11
Deps: 247 (189 direct / 58 transitive)
Vulns: 3"]
    SpringStarter["spring-boot-starter-web:2.7.0"]
    SpringWeb["spring-web:5.3.20
CVE-2024-xxxx"]
    SpringMvc["spring-webmvc:5.3.20"]
    Jackson["jackson-databind:2.13.3"]
    Log4j["log4j-core:2.17.0
CVE-2021-44832"]
    Guava["guava:31.1-jre"]
    Postgres["postgresql:42.3.3"]
    Root --> SpringStarter
    SpringStarter --> SpringWeb
    SpringStarter --> SpringMvc
    SpringStarter --> Jackson
    Root --> Log4j
    Root --> Guava
    Root --> Postgres
```

_Keyboard controls:_ navigate (↑↓), expand (Enter), view details (v), filter (f), search (/), quit (q).
```
