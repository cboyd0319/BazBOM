...
> - **Phase 4 progress (archived)** – see [docs/archive/phases/PHASE_4_PROGRESS.md](../archive/phases/PHASE_4_PROGRESS.md) for historical notes
>
...
flowchart TD
    Header["Header
Project: my-spring-app · shortcuts"]
    Controls["Controls
Search · Filter · Sort"]
    Body["Dependency Panel
Direct & transitive dependencies
Vulnerability indicators"]
    Footer["Footer
Selected dependency details
Keyboard shortcuts"]
    Header --> Controls --> Body --> Footer
```

Key interactions:
- Highlight critical dependencies (Log4Shell, etc.) with quick-fix prompts.
- Show nested transitive dependencies with reachability and KEV status.
- Provide footer summary with severity, scope, license, and shortcuts.
- Support batch selection, search, and filter commands directly from the keyboard.
...
> - jackson-databind: 2.13.0 → 2.16.0 (HIGH)
> - guava: 30.1-jre → 32.1.3-jre (MEDIUM)
> - commons-io: 2.7 → 2.15.0 (MEDIUM)
> - commons-codec: 1.15 → 1.16.0 (LOW)
> - httpclient: 4.5.13 → 4.5.14 (LOW)
> - slf4j-api: 1.7.32 → 2.0.9 (INFO)
>
_Estimated time: ~45 seconds; 127 tests_
...
flowchart TD
    ...
    Overview["Team: Security Engineering<br/>Members: 5"]
    Vulns["Open vulnerabilities: 12<br/>Critical 2 · High 5 · Medium 3 · Low 2"]
    Assignment["Assignments<br/>alice: 3 (1 critical, 2 high)<br/>bob: 2 (high, medium)<br/>carol: 1 (medium)<br/>Unassigned: 6"]
    Metrics["30-day metrics<br/>MTTF 2.3 days<br/>Fixed 24<br/>Introduced 8<br/>Net +16"]
    Contributors["Top contributors<br/>alice 12 fixes<br/>bob 8 fixes<br/>carol 4 fixes"]
    Dashboard --> Overview
    Dashboard --> Vulns
