...
flowchart TD
    TW["BazBOM Security\nTool Window"]
    TW --> Summary["Project summary\n• Project: my-app\n• Build: Maven\n• Last scan: 2 min ago"]
    TW --> Tree["Dependency scopes\nCompile (189)\nTest (58)"]
    Tree --> Log4j["log4j-core 2.17.0\nCVE-2021-44832 · Quick Fix"]
    Tree --> SpringWeb["spring-web 5.3.20\nCritical · KEV · Reachable"]
    Tree --> Jackson["jackson-databind 2.15.2"]
    Tree --> Guava["guava 32.1.1"]
    TW --> SummaryNode["Vulnerabilities: 1 Critical / 1 Medium"]
```
...
```mermaid
flowchart TD
    Settings["BazBOM Settings"]
    Settings --> Toggles["Scanning preferences\n• Real-time\n• Inline warnings\n• Auto-scan on save/open"]
    Settings --> Severity["Severity thresholds\nCritical · High · Medium · Low"]
    Settings --> Policy["Policy file selector"]
    Settings --> CliPath["CLI path selector"]
...
```mermaid
mindmap
  root((bazbom-intellij-plugin))
    src/main/kotlin/io/bazbom/intellij
      BazBomPlugin.kt
      settings
        BazBomSettings.kt
        BazBomConfigurable.kt
      toolwindow
        DependencyTreeWindow.kt
        DependencyNodeRenderer.kt
      annotator
        MavenDependencyAnnotator.kt
        GradleDependencyAnnotator.kt
        BazelDependencyAnnotator.kt
      quickfix
        UpgradeDependencyQuickFix.kt
```
...
flowchart LR
    ...
    CLI["bazbom CLI\n(Security engine)"]
    VSCode -- LSP protocol --> LSP
    LSP -- Executes scans --> CLI
    CLI -- Findings & diagnostics --> LSP
    LSP -- Publish diagnostics --> VSCode
```
...
**Current State:** Phase 4 not started (docs/archive/phases/PHASE_4_PROGRESS.md:304)
