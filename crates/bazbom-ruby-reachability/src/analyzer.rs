//! Main Ruby reachability analyzer
...
impl RubyReachabilityAnalyzer {
    ...
    pub fn analyze(&mut self) -> Result<ReachabilityReport> {
        info!("Starting Ruby reachability analysis");
        ...
        if self.has_dynamic_code {
            warn!("Dynamic code detected in Ruby project - using conservative analysis");
    ...
    fn process_file(&mut self, file_path: &Path) -> Result<()> {
        ...
        for func in &extractor.functions {
            ...
            let function_node = FunctionNode {
                ...
                is_instance_method: func.is_instance_method,
...
"#;
        ...
        fs::write(temp_dir.path().join("main.rb"), code).unwrap();
    ...
    fn test_analyze() {
        ...
        let mut analyzer = RubyReachabilityAnalyzer::new(temp_dir.path().to_path_buf());
    ...
    fn test_detect_entrypoints() {
        let temp_dir = create_test_project();
        let analyzer = RubyReachabilityAnalyzer::new(temp_dir.path().to_path_buf());

        let entrypoints = analyzer.detect_entrypoints().unwrap();

        // Simple Ruby files don't have explicit entrypoints
