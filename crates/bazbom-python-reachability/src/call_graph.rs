...
impl CallGraph {
    ...
    pub fn add_call(&mut self, caller_id: &str, callee_id: &str) -> Result<()> {
        let caller_idx = self
            ...
            .get(caller_id)
            .ok_or_else(|| {
                PythonReachabilityError::CallGraphError(format!("Caller not found: {}", caller_id))
            })?;
        ...
        let callee_idx = self
            ...
            .get(callee_id)
            .ok_or_else(|| {
                PythonReachabilityError::CallGraphError(format!("Callee not found: {}", callee_id))
            })?;
    ...
    pub fn mark_entrypoint(&mut self, function_id: &str) -> Result<()> {
        self.functions
            .get_mut(function_id)
            .ok_or_else(|| {
                PythonReachabilityError::CallGraphError(format!(
                    "Function not found: {}",
                    function_id
                ))
            })?
