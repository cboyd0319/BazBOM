...
fn test_sarif_result_with_location() {
    let result =
        SarifResult::new("CVE-2023-5678", "error", "Outdated dependency").with_location("pom.xml");

    assert_eq!(result.locations.as_ref().unwrap().len(), 1);
    assert_eq!(
        result.locations.as_ref().unwrap()[0]
            .physical_location
            .artifact_location
            .uri,
        "pom.xml"
...
fn test_sarif_with_multiple_locations() {
    let result1 =
        SarifResult::new("CVE-2023-0001", "error", "Vuln in POM").with_location("pom.xml");
    let result2 =
        SarifResult::new("CVE-2023-0002", "error", "Vuln in Gradle").with_location("build.gradle");

    assert_eq!(
        result1.locations.as_ref().unwrap()[0]
            .physical_location
            .artifact_location
            .uri,
        "pom.xml"
    ...
    assert_eq!(
        result2.locations.as_ref().unwrap()[0]
            .physical_location
            .artifact_location
            .uri,
        "build.gradle"
