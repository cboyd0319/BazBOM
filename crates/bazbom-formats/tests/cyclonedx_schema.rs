...
fn test_cyclonedx_add_component() {
    ...
    let component = Component::new("example-lib", "library").with_version("1.0.0");
...
fn test_cyclonedx_component_with_purl() {
    ...
    assert_eq!(
        component.purl,
        Some("pkg:maven/com.example/test-lib@2.0.0".to_string())
    );
}
...
fn test_cyclonedx_deserialization() {
    ...
    let bom: CycloneDxBom =
        serde_json::from_str(json).expect("Failed to deserialize CycloneDX BOM");
