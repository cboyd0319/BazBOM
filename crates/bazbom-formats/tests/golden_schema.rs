...
fn test_spdx_minimal_golden() {
    ...
    // Parse golden file
    let golden: SpdxDocument = serde_json::from_str(&golden_content)
        .expect("Failed to parse golden SPDX document");
    ...
    let serialized = serde_json::to_string_pretty(&golden)
        .expect("Failed to serialize SPDX document");
    
    // Parse again to ensure round-trip consistency
    let _roundtrip: SpdxDocument = serde_json::from_str(&serialized)
        .expect("Failed to parse round-trip SPDX");
}
...
fn test_cyclonedx_minimal_golden() {
    ...
    // Parse golden file
    let golden: CycloneDxBom = serde_json::from_str(&golden_content)
        .expect("Failed to parse golden CycloneDX BOM");
    ...
    // Parse again to ensure round-trip consistency
    let _roundtrip: CycloneDxBom = serde_json::from_str(&serialized)
        .expect("Failed to parse round-trip CycloneDX");
}
...
fn test_sarif_minimal_golden() {
    ...
    // Parse golden file
    let golden: SarifReport = serde_json::from_str(&golden_content)
        .expect("Failed to parse golden SARIF report");
    
    // Verify structure
    assert_eq!(golden.version, "2.1.0");
    assert_eq!(golden.schema, "https://json.schemastore.org/sarif-2.1.0.json");
    ...
    // Parse again to ensure round-trip consistency
    let _roundtrip: SarifReport = serde_json::from_str(&serialized)
        .expect("Failed to parse round-trip SARIF");
}
...
fn test_spdx_schema_compliance() {
    ...
    let _parsed: SpdxDocument = serde_json::from_str(&json)
        .expect("Failed to parse generated SPDX");
}
...
fn test_cyclonedx_schema_compliance() {
    ...
    // Parse back to ensure valid
    let _parsed: CycloneDxBom = serde_json::from_str(&json)
        .expect("Failed to parse generated CycloneDX");
}
...
fn test_sarif_schema_compliance() {
    ...
    // Parse back to ensure valid
    let _parsed: SarifReport = serde_json::from_str(&json)
        .expect("Failed to parse generated SARIF");
}
