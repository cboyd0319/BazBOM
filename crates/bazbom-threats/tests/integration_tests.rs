...
fn test_sync_with_fallback() {
    ...
    assert!(db.packages.contains_key("maven") || db.packages.contains_key("npm") || db.packages.is_empty());
}
...
fn test_malicious_keyword_detection() {
    ...
    for (text, expected_malicious) in test_cases {
        ...
        assert_eq!(
            is_malicious, expected_malicious,
            "Failed for: {}",
            text
        );
    }
...
fn test_database_persistence() {
    ...
    db.add_entry(bazbom_threats::database_integration::MaliciousPackageEntry {
...
fn test_real_osv_api() {
    ...
    let mut sync = ThreatDatabaseSync::new();
    let result = sync.sync_osv(&["MAVEN"]);
    
    // This test requires network and OSV API to be available
    if result.is_ok() {
        let db = sync.database();
        println!("Synced {} Maven packages from OSV", 
...
fn test_real_ghsa_api() {
    ...
    if std::env::var("GITHUB_TOKEN").is_ok() {
        let result = sync.sync_ghsa(&["MAVEN"]);
        
        if result.is_ok() {
            let db = sync.database();
            println!("Synced {} Maven packages from GHSA", 
                     db.get_malicious_packages("maven").len());
        } else {
