...
pub fn load_cached_result(
    ...
    let result: ReachabilityResult = serde_json::from_str(&result_content)
            ...
            if let Ok(metadata) = serde_json::from_str::<CacheMetadata>(&content) {
                if let Ok(timestamp) = chrono::DateTime::parse_from_rfc3339(&metadata.timestamp) {
                    if timestamp.with_timezone(&chrono::Utc) < cutoff {
                        ...
                        fs::remove_file(&path)?;
                        let result_path_str = path.to_string_lossy()
...
mod tests {
    ...
    fn test_cache_miss_on_different_inputs() {
        ...
        let loaded = load_cached_result(&cache_dir, "/path/to/other.jar", "com.example.Main")
            .unwrap();
        
        assert!(loaded.is_none(), "Should not find cached result with different classpath");
    }
