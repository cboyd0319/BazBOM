...
impl OpenRewriteRunner {
    ...
    pub fn generate_recipes(&self, ctx: &Context, vulnerabilities: &[VulnerabilityFinding]) -> Result<Vec<OpenRewriteRecipe>> {
        if !self.is_enabled() {
            return Ok(Vec::new());
        }
        ...
        if !recipes.is_empty() {
            ...
            std::fs::write(&recipes_file, json)
                .context("failed to write OpenRewrite recipes")?;
            
            println!("[bazbom] wrote {} OpenRewrite recipes to {:?}", recipes.len(), recipes_file);
    ...
    fn generate_dry_run_patches(&self, ctx: &Context, recipes: &[OpenRewriteRecipe]) -> Result<()> {
        println!("[bazbom] generating dry-run patches for {} recipes", recipes.len());
