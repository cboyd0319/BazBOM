...
fn create_test_app(dir: &Path) -> std::io::Result<()> {
    ...
    writeln!(file, "        System.out.println(\"This should not be reachable\");")?;
    writeln!(file, "    }}")?;
    writeln!(file, "}}")?;
    ...
    writeln!(file, "        System.out.println(\"This class is never used\");")?;
    writeln!(file, "    }}")?;
    writeln!(file, "}}")?;
...
fn test_end_to_end_reachability_workflow() {
    ...
    if !reachability_jar.exists() {
        eprintln!("Skipping test: reachability JAR not found at {:?}", reachability_jar);
    ...
    let entrypoint_strs: Vec<String> = entrypoints
        ...
        .collect();
    assert!(
        entrypoint_strs.iter().any(|e| e.contains("Main") && e.contains("main")),
    ...
    assert!(!reachable_methods.is_empty(), "Should have reachable methods");
    ...
    assert!(
        method_strs.iter().any(|m| m.contains("Main") && m.contains("main")),
        "Main.main method should be reachable"
    );
    
    // Helper.doSomething should be reachable
    assert!(
        method_strs.iter().any(|m| m.contains("Helper") && m.contains("doSomething")),
        "Helper.doSomething should be reachable"
    );
...
fn test_reachability_with_cache() {
    ...
    assert_eq!(
        result1["reachableClasses"],
        result2["reachableClasses"],
        "Both runs should find same reachable classes"
    );
    assert_eq!(
        result1["reachableMethods"],
        result2["reachableMethods"],
